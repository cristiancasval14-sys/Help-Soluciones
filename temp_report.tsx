...
export default function ServiceReports() {
    ...
    const filteredInventory = inventory.filter(inv => {
        ...
        // Mostrar estrictamente el equipo que est+í asignado a la persona seleccionada
                ...
                <div style={{
                    ...
                    {ticketContext.client && (
                        <span style={{ fontSize: '0.875rem', color: 'var(--text-muted)' }}>
                            ­ƒÅó <strong>{ticketContext.client}</strong>
                        </span>
                    )}
                    {ticketContext.requester && (
                        <span style={{ fontSize: '0.875rem', color: 'var(--text-muted)' }}>
                            ­ƒæñ <strong>{ticketContext.requester}</strong>
                        </span>
                    )}
                    ...
                    {ticketContext.category && (
                        <span style={{ fontSize: '0.875rem', color: 'var(--text-muted)' }}>­ƒôé {ticketContext.category}</span>
                    )}
                ...
                <div className="card glass" style={{ gridColumn: 'span 8', padding: '2rem' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '1.5rem', marginBottom: '1.5rem' }}>
                        ...
                        <div className="form-group">
                            ...
                            <select name="modality" value={formData.modality} onChange={handleInputChange} className="form-input" style={{ fontWeight: 600, color: 'var(--primary)', padding: '0.8rem 0.5rem' }}>
                                <option value="Soporte Remoto">­ƒÄº Remoto</option>
                                <option value="Visita T+®cnica Programada">­ƒôà Visita</option>
                                <option value="Presencial Emergencia">­ƒÜ¿ Emergencia</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div className="form-group" style={{ marginBottom: '2rem' }}>
                        ...
                        <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '4px' }}>
                            {formData.activitySummary.length} caracteres -À Use este campo para documentar todo lo ejecutado durante la visita o soporte.
                        </p>
                    </div>
                ...
                <div style={{ gridColumn: 'span 4', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                                    ...
                                    <div className="fade-in">
                                        ...
                                        {selectedAsset.licenses?.length > 0 && (
                                            <div style={{ marginTop: '0.8rem', padding: '0.5rem 0.8rem', borderRadius: '8px', background: 'rgba(99,102,241,0.06)', border: '1px solid rgba(99,102,241,0.15)', fontSize: '0.78rem', color: 'var(--primary)', fontWeight: 600 }}>
                                                ­ƒöæ {selectedAsset.licenses.length} licencia{selectedAsset.licenses.length !== 1 ? 's' : ''} registrada{selectedAsset.licenses.length !== 1 ? 's' : ''}
                                            </div>
                                        )}
                                    </div>
                                )}
                    ...
                    <div className="card glass">
                        ...
                        <div className="form-group" style={{ marginBottom: '2rem' }}>
                            ...
                            >
                                <option value="Si">Ô£à S+¡, Solucionado (Cerrar caso)</option>
                                <option value="Parcial">ÔÜá´©Å Soluci+¦n Parcial (En monitoreo)</option>
                                <option value="No">ÔØî No Solucionado (Escalar o Reprogramar)</option>
                            </select>
                        </div>
                    ...
                    <div className="card glass" style={{ background: 'rgba(37,99,235,0.03)', border: '1px solid var(--primary-glow)' }}>
                        ...
                        <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                            Al guardar este informe, quedar+í enlazado en el <strong>historial del equipo</strong> en el inventario.
                        </p>
                    </div>
                </div>
            </form>
                         ...
                         <div id="printable-report" style={{ padding: '3.5rem' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0', marginBottom: '3.5rem' }}>
                                ...
                                <div style={{ borderLeft: '3px solid #3b82f6', paddingLeft: '1.2rem', minHeight: '60px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                                    ...
                                    <div>
                                        <p style={{ fontSize: '0.75rem', fontWeight: 800, textTransform: 'uppercase', color: '#64748b', marginBottom: '6px', letterSpacing: '0.05em' }}>T+ëCNICO</p>
                                    ...
                                    <div style={{ border: '1px solid #e2e8f0', borderRadius: '16px', overflow: 'hidden', background: 'white' }}>
                                            ...
                                            <p style={{ margin: 0, display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                                ...
                                                <span style={{ fontSize: '1rem', color: '#1e293b', fontWeight: 700 }}>
                                                    {selectedReport.inventory
                                                        ? `${selectedReport.inventory.equipment_id} ÔÇö ${selectedReport.inventory.brand} ${selectedReport.inventory.model}`
                                        ...
                                        <div style={{ padding: '1.25rem' }}>
                                            <p style={{ margin: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                ...
                                                <span style={{ fontSize: '1.1rem', color: '#10b981', fontWeight: 800 }}>Ô£ô Realizado</span>
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: '20px', padding: '2.5rem', minHeight: '350px', fontSize: '1rem', color: '#334155', lineHeight: 2 }}>
                                        <div style={{ whiteSpace: 'pre-wrap' }}>
                                            {selectedReport.activities.split('\n').map((line: string, i: number) => {
                                                ...
                                                if (lower.includes('contacto')) processed = '­ƒô× ' + line;
                                                else if (lower.includes('ubicaci+¦n') || lower.includes('sede')) processed = '­ƒôì ' + line;
                                                else if (lower.includes('equipo')) processed = '­ƒÆ+ ' + line;
                                                else if (lower.includes('acci+¦n') || lower.includes('realiz+¦')) processed = 'ÔÜí ' + line;
                                                else if (line.trim()) processed = 'ÔÇó ' + line;
                            ...
                            <footer style={{ marginTop: '6rem', paddingTop: '2.5rem', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                                <div style={{ fontSize: '0.8rem', color: '#94a3b8' }}>
                                    <p style={{ margin: 0 }}>-® 2026 Help Soluciones Inform+íticas S.A.S</p>
